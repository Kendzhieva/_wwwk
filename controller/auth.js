...
export const register = async (req, res) => {
    try {
        ...
        await sendActivationMail(user.email, `http://localhost:3333/active/${activeLink}`)
        ...
        console.log(err)
        res.status(500).json({
            message: 'Не удалось засуществоватся'
        })
    }
}
...
export const login = async (req, res) => {
    try {
        ...
        console.log(err)
        res.status(500).json({
            message: 'Не удалось войти в существования'
        })
    }
